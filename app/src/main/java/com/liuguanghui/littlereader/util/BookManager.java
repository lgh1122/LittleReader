...
public class BookManager {
    ...
    public boolean openChapter(Long netId,Long novelId,Long chapterId, String chapterName) {
        return openChapter(netId,novelId,chapterId, chapterName, 0);
    }

    public boolean openChapter(Long netId,Long novelId, Long chapterId, String chapterName, long position) {
        //如果文件不存在，则打开失败
        File file = new File(Constant.BOOK_CACHE_PATH + netId+File.separator +novelId
                + File.separator + chapterId + FileUtils.SUFFIX_WY);
        if (!file.exists()) {
            return false;
        }
        this.netId = netId;
    ...
    private void createCache() {
        ...
        if (!cacheMap.containsKey(chapterName)) {
            ...
            File file = getBookFile(netId,novelId,chapterId);
    ...
    public static File getBookFile(Long netId, Long novelId, Long chapterId) {
        return FileUtils.getFile(Constant.BOOK_CACHE_PATH + netId+File.separator +novelId
                + File.separator + chapterId+ FileUtils.SUFFIX_WY);
    ...
    public static boolean isChapterCached(Long netId, Long bookId, Long id) {
        File file = new File(Constant.BOOK_CACHE_PATH + netId+File.separator +bookId
                + File.separator + id + FileUtils.SUFFIX_WY);
        return file.exists();
    }
